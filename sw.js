...
<body>
  ...
  <!-- Modals -->
  <div class="overlay" id="aboutModal"><div class="modal"><div class="about-brand"><div class="about-logo"><svg viewBox="0 0 24 24" fill="white"><path d="M12 2C12 2 5 10.5 5 15C5 18.866 8.134 22 12 22C15.866 22 19 18.866 19 15C19 10.5 12 2 12 2Z"/></svg></div><div class="about-name">DropLit</div><div class="about-ver">v0.6.2 by Syntrise</div></div><div class="modal-text" style="text-align:center">Voice-first idea capture.<br>Long press card to select & merge.</div><div class="modal-actions"><button class="modal-btn pri" onclick="closeAbout()">Got it</button></div></div></div>
...
const CATS={tasks:{name:'TASKS',kw:['задача','сделать','нужно','надо','task','todo']},ideas:{name:'IDEAS',kw:['идея','мысль','придумал','idea']},handmagic:{name:'HANDMAGIC',kw:['handmagic','хендмеджик','магия']},design:{name:'DESIGN',kw:['дизайн','ui','ux','кнопка','цвет','design']},bugs:{name:'BUGS',kw:['баг','ошибка','bug','fix']},questions:{name:'QUESTIONS',kw:['вопрос','спросить','claude']},inbox:{name:'INBOX',kw:[]}};
...
function cardTouchStart(id,e){
  ...
  longPressTimer=setTimeout(()=>handleCardLongPress(id),700);
}
...
function render(){
  ...
  for(const d of dates){
    ...
    for(const i of dayIdeas){
      ...
      h+='<div class="card'+(isActive?' active':'')+(isEditing?' editing':'')+(isSelected?' selected':'')+'" data-id="'+i.id+'" onclick="handleCardClick('+i.id+',event)" ontouchstart="cardTouchStart('+i.id+',event)" ontouchend="cardTouchEnd()" ontouchcancel="cardTouchEnd()" oncontextmenu="return false;">';
      h+='<div class="card-checkbox">'+(isSelected?'✓':'')+'</div>';
      h+='<div class="card-head"><span class="card-cat '+i.category+'" onclick="openCatModal('+i.id+')">'+CATS[i.category].name+'</span>';
      h+='<span class="card-chars">'+cc+'</span>';
      h+='<span class="card-time">'+smartTime(i)+'</span></div>';
      h+='<div class="card-text">'+esc(i.text)+'</div>';
