...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
    ...
    /* Header */
    .header { display: flex; align-items: center; justify-content: space-between; padding: 10px 16px; background: var(--color-bg-card); position: relative; padding-top: max(10px, env(safe-area-inset-top)); z-index: 100; box-shadow: 0 2px 8px rgba(0,0,0,0.08); }
    ...
    .filters { max-height: 0; overflow: hidden; opacity: 0; transition: max-height 0.3s ease, opacity 0.2s ease; border-bottom: none; }
    body.filters-visible .filters { max-height: 200px; opacity: 1; border-bottom: 1px solid var(--color-border); }
    ...
    /* Filters */
    .filters { background: var(--color-bg-card); padding: 8px 10px; }
    ...
    .speaking-indicator { margin-left: 10px; font-size: 1.2rem; opacity: 0.5; cursor: pointer; transition: all 0.2s; }
    .speaking-indicator.active { opacity: 1; animation: speakPulse 1s infinite; }
...
<body>
    ...
    <div class="ai-result-content" style="max-width: 360px;">
      ...
      <div class="ai-result-body" style="padding: 20px;">
        ...
        <div class="translate-langs">
          ...
          <button class="translate-lang-btn" onclick="runTranslate('Russian')">🇷🇺 Русский</button>
          <button class="translate-lang-btn" onclick="runTranslate('Spanish')">🇪🇸 Español</button>
          <button class="translate-lang-btn" onclick="runTranslate('German')">🇩🇪 Deutsch</button>
          <button class="translate-lang-btn" onclick="runTranslate('French')">🇫🇷 Français</button>
          <button class="translate-lang-btn" onclick="runTranslate('Chinese')">🇨🇳 中文</button>
          <button class="translate-lang-btn" onclick="runTranslate('Japanese')">🇯🇵 日本語</button>
          <button class="translate-lang-btn" onclick="runTranslate('Portuguese')">🇧🇷 Português</button>
        </div>
      </div>
  ...
  <div class="ask-ai-panel" id="askAIPanel">
    ...
    <div class="ask-ai-header">
      <div class="ask-ai-header-left">
        <div class="ask-ai-avatar">✨</div>
        <div>
          <div class="ask-ai-title">Aski</div>
          <div class="ask-ai-subtitle">AI Chat</div>
        </div>
      </div>
    ...
    <div class="ask-ai-messages" id="askAIMessages">
      <div class="ask-ai-empty" id="askAIEmpty">
        <div class="ask-ai-empty-icon">✨</div>
        ...
        <div class="ask-ai-prompts">
          <button class="ask-ai-prompt" onclick="setAskAIPrompt('Что приготовить на ужин?')">🍳 Что приготовить?</button>
          <button class="ask-ai-prompt" onclick="setAskAIPrompt('Напиши поздравление')">🎂 Поздравление</button>
          <button class="ask-ai-prompt" onclick="setAskAIPrompt('Как организовать день?')">📅 План дня</button>
        </div>
      </div>
    </div>
      ...
      <div class="menu-section no-border">
        ...
        <div class="settings-item no-border" style="flex-direction: column; align-items: flex-start; gap: 12px;">
          <div class="settings-item-left">
            ...
            <span class="settings-item-label">Aski voice</span>
          </div>
          <div class="pill-row" id="voiceGenderSelector">
            <button class="pill-m active" data-gender="female" onclick="setAskiVoiceGender('female')">Female</button>
            <button class="pill-m" data-gender="male" onclick="setAskiVoiceGender('male')">Male</button>
          </div>
        </div>
        ...
        <div class="menu-about" onclick="closeMainMenu(); openAbout();" style="cursor: pointer;">
          ...
          <div class="menu-about-ver">v0.9.8 by Syntrise</div>
  ...
  <!-- Modals -->
  <div class="overlay" id="aboutModal"><div class="modal"><div class="about-brand"><div class="about-logo"><svg viewBox="0 0 24 24" fill="white"><path d="M12 2C12 2 5 10.5 5 15C5 18.866 8.134 22 12 22C15.866 22 19 18.866 19 15C19 10.5 12 2 12 2Z"/></svg></div><div class="about-name">DropLit</div><div class="about-ver">v0.9.8 by Syntrise</div></div><div class="modal-text" style="text-align:center">Voice-first idea capture.<br>Long press card to select & merge.</div><div class="modal-actions"><button class="modal-btn pri" onclick="closeAbout()">Got it</button></div></div></div>
...
function openAskAI() {
  document.getElementById('askAIPanel').classList.add('show');
...
function closeAskAI() {
  document.getElementById('askAIPanel').classList.remove('show');
...
let askiVoiceGender = localStorage.getItem('aski_voice_gender') || 'female'; // 'female' or 'male'
...
function getVoiceForLang(lang) {
  ...
  const preferFemale = askiVoiceGender === 'female';
...
function askiSpeak(text, lang = null, onEnd = null) {
  if (askiIsSpeaking) {
    askiStopSpeaking();
  }
  ...
  const detectedLang = lang || detectLanguage(cleanText);
  
  const utterance = new SpeechSynthesisUtterance(cleanText);
  utterance.lang = detectedLang;
  utterance.rate = 0.95; // Slightly slower for clarity
  utterance.pitch = askiVoiceGender === 'female' ? 1.1 : 0.9;
  ...
  utterance.onerror = (e) => {
    ...
    askiCurrentUtterance = null;
    updateSpeakingIndicator(false);
    if (onEnd) onEnd(); // Call onEnd even on error
  };
...
function updateSpeakingIndicator(isSpeaking) {
  ...
  let indicator = header.querySelector('.speaking-indicator');
  ...
  if (isSpeaking) {
    if (!indicator) {
      ...
      indicator.innerHTML = '🔊';
      indicator.onclick = askiStopSpeaking;
      header.querySelector('.ask-ai-header-left').appendChild(indicator);
    }
...
let voiceModeSleeping = false; // Sleep mode after timeout
let voiceModeTimeout = null;  // Timer for sleep mode
let voiceModeSilentCycles = 0; // Count silent "no-speech" cycles
const VOICE_MODE_TIMEOUT_MS = 30000; // 30 seconds of silence before sleep
const VOICE_MODE_MAX_SILENT_CYCLES = 2; // Go to sleep after 2 silent cycles (~6-10 sec)
...
function setAutoSpeak(enabled) {
  ...
  showToast(enabled ? 'Auto-speak enabled' : 'Auto-speak disabled');
}
...
function startVoiceModeListening() {
  ...
  // Start/reset the sleep timeout
  resetVoiceModeTimeout();
  ...
  voiceModeRecognition.onresult = (event) => {
    ...
    if (transcript.trim()) {
      // User spoke real words - reset silent counter
      voiceModeSilentCycles = 0;
      clearVoiceModeTimeout();
      ...
      // Empty result = noise without words = count as silence
      voiceModeSilentCycles++;
      console.log('Empty transcript, silent cycle:', voiceModeSilentCycles);
      if (voiceModeSilentCycles >= VOICE_MODE_MAX_SILENT_CYCLES) {
        enterVoiceModeSleep();
      }
    }
  };
  ...
  voiceModeRecognition.onend = () => {
    ...
    // Don't restart if sleeping, locked, or page hidden
    if (voiceModeSleeping || voiceModeLocked || document.hidden) {
      return;
    }
    
    // This fires after successful recognition - restart normally
    if (isVoiceModeEnabled() && 
        document.getElementById('askAIPanel')?.classList.contains('show')) {
      updateVoiceModeIndicator('waiting');
      setTimeout(() => {
        if (!voiceModeLocked && !voiceModeSleeping && !askiIsSpeaking && !askiIsProcessing && !document.hidden) {
  ...
  voiceModeRecognition.onerror = (e) => {
    ...
    // Count silent cycles (no-speech errors)
    if (e.error === 'no-speech') {
      voiceModeSilentCycles++;
      console.log('Silent cycle:', voiceModeSilentCycles, '/', VOICE_MODE_MAX_SILENT_CYCLES);
      
      // After N silent cycles, go to sleep
      if (voiceModeSilentCycles >= VOICE_MODE_MAX_SILENT_CYCLES) {
        enterVoiceModeSleep();
        return;
      }
    } else if (e.error !== 'aborted') {
      console.error('Voice mode error:', e.error);
    }
    
    // Don't restart if sleeping or locked
    if (voiceModeSleeping || voiceModeLocked || document.hidden) {
      return;
    }
...
// Reset the sleep timeout (call when starting to listen)
function resetVoiceModeTimeout() {
  clearVoiceModeTimeout();
  voiceModeTimeout = setTimeout(() => {
    enterVoiceModeSleep();
...
function enterVoiceModeSleep() {
  console.log('Voice mode entering sleep (no words detected)');
  voiceModeSleeping = true;
  voiceModeSilentCycles = 0; // Reset counter
  stopVoiceModeListening();
  clearVoiceModeTimeout();
  updateVoiceModeIndicator('sleeping');
}
...
function wakeVoiceMode() {
  console.log('Voice mode waking up');
  voiceModeSleeping = false;
  voiceModeLocked = false;
  voiceModeSilentCycles = 0; // Reset silent counter
  updateVoiceModeIndicator('waiting');
...
function updateVoiceModeIndicator(state) {
  const subtitle = document.querySelector('.ask-ai-subtitle');
  if (!subtitle) return;
  
  switch(state) {
    case 'listening':
      subtitle.textContent = '🎤 Listening...';
      subtitle.style.color = '#10B981'; // Green
      break;
    case 'processing':
      subtitle.textContent = '🤔 Thinking...';
      subtitle.style.color = '#F59E0B'; // Orange
      break;
    case 'speaking':
      subtitle.textContent = '🔊 Speaking...';
      subtitle.style.color = '#06B6D4'; // Cyan
      break;
    case 'locked':
      subtitle.textContent = '⏳ Please wait...';
      subtitle.style.color = '#8B5CF6'; // Purple
      break;
    case 'waiting':
      ...
      break;
    case 'sleeping':
      subtitle.textContent = '💤 Tap mic to talk';
      subtitle.style.color = '#6B7280'; // Gray
      break;
    default:
      subtitle.textContent = 'AI Chat';
      subtitle.style.color = '';
  }
}

// Set voice gender
function setAskiVoiceGender(gender) {
  askiVoiceGender = gender;
  localStorage.setItem('aski_voice_gender', gender);
  // Update UI
  document.querySelectorAll('#voiceGenderSelector .pill-m').forEach(btn => {
  ...
  if (isUser) {
    ...
    msgDiv.innerHTML = `
      ...
      <div class="ask-ai-actions">
        ...
        <button class="ask-ai-action-btn primary" onclick="createDropFromAI(this)">
  ...
  try {
    const response = await fetch(AI_API_URL, {
      ...
      body: JSON.stringify({
        action: 'chat',
        text: text,
        history: askAIMessages.slice(-10), // Last 10 messages for context
        syntriseContext: syntriseContext // RAG context from Syntrise CORE
      })
    });
    ...
    addAskAIMessage('Connection error. Please check your internet connection.', false);
    // Unlock on error
    unlockVoiceMode();
  }
}
...
function createDropFromAI(btn) {
  ...
  showToast('Drop created from AI response');
}
...
function toggleAskAIVoice() {
  ...
  if (isVoiceModeEnabled()) {
    ...
    if (voiceModeLocked || askiIsProcessing || askiIsSpeaking) {
      showToast('Wait for Aski to finish');
      return;
    }
...
function createDropFromResult() {
  ...
  let category = 'ideas';
  if (title.includes('Poem') || title.includes('Стих')) category = 'ideas';
  if (title.includes('Speech') || title.includes('Спич')) category = 'ideas';
  if (title.includes('Greeting') || title.includes('Поздрав')) category = 'ideas';
...
function toggleVoiceMode() {
  ...
  if (isEnabled) {
    showToast('🎤 Voice Mode ON — speak and Aski will respond by voice');
    acquireWakeLock(); // Keep screen on
    // If chat is open, start listening
...
function initVoiceSettings() {
  ...
  // Voice Gender
  const savedGender = localStorage.getItem('aski_voice_gender') || 'female';
  document.querySelectorAll('#voiceGenderSelector .pill-m').forEach(btn => {
...
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    // Page is hidden - stop voice mode listening to prevent loops
    if (voiceModeRecognition) {
      stopVoiceModeListening();
      console.log('Voice mode paused (page hidden)');
    }
  } else {
    // Page is visible again - resume voice mode if enabled and chat is open
    if (isVoiceModeEnabled() && 
        document.getElementById('askAIPanel')?.classList.contains('show') &&
        !voiceModeLocked && !askiIsSpeaking && !askiIsProcessing) {
      setTimeout(startVoiceModeListening, 500);
      console.log('Voice mode resumed (page visible)');
    }
  }
});
