...
<body>
    ...
    <div class="audio-recorder-content">
      ...
      <div class="audio-recorder-body">
        ...
        <div class="audio-recorder-actions">
          <button class="act-btn delete" onclick="recorderDelete()">DELETE</button>
          <button class="act-btn create" onclick="recorderCreateDrop()">CREATE DROP</button>
          <button class="act-btn share" onclick="recorderShare()">SHARE</button>
        </div>
    ...
    <div class="main-menu-content">
      ...
      <div class="menu-section no-border">
        ...
        <div class="menu-about" onclick="closeMainMenu(); openAbout();" style="cursor: pointer;">
          ...
          <div class="menu-about-ver">v0.9.32 by Syntrise</div>
  ...
  <!-- Modals -->
  <div class="overlay" id="aboutModal"><div class="modal"><div class="about-brand"><div class="about-logo"><svg viewBox="0 0 24 24" fill="white"><path d="M12 2C12 2 5 10.5 5 15C5 18.866 8.134 22 12 22C15.866 22 19 18.866 19 15C19 10.5 12 2 12 2Z"/></svg></div><div class="about-name">DropLit</div><div class="about-ver">v0.9.32 by Syntrise</div></div><div class="modal-text" style="text-align:center">Voice-first idea capture.<br>Long press card to select & merge.</div><div class="modal-actions"><button class="modal-btn pri" onclick="closeAbout()">Got it</button></div></div></div>
...
async function startRecorderRecording() {
  try {
    ...
    audioRecorder.ondataavailable = (e) => {
      if (e.data.size > 0) audioRecordingChunks.push(e.data);
    };
...
function stopRecorderRecording() {
...
async function recorderCreateDrop() {
  ...
  stopRecorderRecording();
  showToast('Saving audio...');
  await saveAudioDrop(recorderBlob);
  closeAudioRecorder();
}

async function recorderShare() {
  if (!recorderBlob) {
    showToast('No recording to share');
    return;
  }
  
  if (navigator.share) {
    try {
      const file = new File([recorderBlob], 'recording.webm', { type: recorderBlob.type });
...
async function previewVoice(voice) {
  if (!askiApiKey) {
    showToast('Enter API key to preview');
    return;
  }
...
function setTTSProvider(provider) {
  ...
  showToast(`TTS: ${provider === 'openai' ? 'OpenAI' : provider === 'elevenlabs' ? 'ElevenLabs' : 'Browser'}`);
}
...
async function previewElevenLabsVoice(voice) {
  ...
  if (!elevenlabsApiKey) {
    showToast('Enter ElevenLabs API key');
    return;
  }
...
function setListenCycles(seconds) {
  ...
  showToast(`Listen time: ${seconds} sec`);
}
...
function createDropFromAI(btn) {
  ...
  if (btn.classList.contains('created')) {
    showToast('Drop already created');
    return;
  }
  ...
  const drop = {
    id: Date.now(),
    text: text,
    category: isUserMessage ? 'idea' : 'ai',
    timestamp: new Date().toISOString(),
    date: new Date().toLocaleDateString('ru-RU'),
    time: new Date().toLocaleTimeString('ru-RU', {hour:'2-digit', minute:'2-digit'}),
    isMedia: false
...
function setAutoDrop(enabled) {
  ...
  showToast(enabled ? 'AutoDrop enabled' : 'AutoDrop disabled');
}
