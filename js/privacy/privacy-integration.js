...
async function initPrivacySystem(options = {}) {
  ...
  try {
    ...
    if (PrivacySystem.config.enableEncryption) {
      ...
      if (window.DropLitCrypto && masterKey) {
        PrivacySystem.layers.encryption.module = window.DropLitCrypto;
...
export {
  ...
  // Drop processing
  processDropForStorage,
  processDropFromStorage,
