...
function setupModalEventListeners(userId) {
  ...
  setupBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (selectedMethod === 'password') {
        result = await window.SyncEncrypted.setupEncryptionWithPassword(userId, passwordInput.value);
      } else {
        result = await window.SyncEncrypted.setupEncryptionRandom(userId);
      }
...
function updateEncryptionIndicator() {
  ...
  const isReady = window.SyncEncrypted?.isReady || false;
