...
async function initEncryption() {
  ...
  const hasKey = await window.CryptoKeys.hasStoredKey(user.id);
  ...
  // Load key
  const keyData = await window.CryptoKeys.retrieveKey(user.id);
  if (!keyData) {
...
async function setupEncryptionWithPassword(userId, password) {
  const result = await window.CryptoKeys.initializeEncryption(userId, 'password', password);
  
  if (result.success) {
    const keyData = await window.CryptoKeys.retrieveKey(userId);
    encryptionKey = keyData.key;
    encryptionReady = true;
...
async function setupEncryptionRandom(userId) {
  const result = await window.CryptoKeys.initializeEncryption(userId, 'random');
...
async function syncDropEncrypted(drop, action = 'upsert') {
  ...
  try {
    ...
    const prepared = await window.DropEncryption.prepareDropForSync(drop, encryptionKey);
...
async function fetchDropsEncrypted() {
  ...
  try {
    ...
    for (const serverDrop of data) {
      const drop = await window.DropEncryption.processDropFromServer(serverDrop, encryptionKey);
      decrypted.push(drop);
    }
...
async function migrateLocalDrops() {
  ...
  const result = await window.DropEncryption.migrateAllDrops(encryptionKey);
