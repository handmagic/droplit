...
<body>
  ...
  <div class="insights-banner" id="insightsBanner">
    ...
    <div class="insights-banner-content">
      <div class="insights-banner-title" id="insightTitle">Напоминание</div>
  ...
  <div class="notif-banner" id="notifBanner">
    <div class="notif-banner-title">🔔 Включить уведомления?</div>
    <div class="notif-banner-text">ASKI сможет напоминать о днях рождения и важных событиях</div>
    <div class="notif-banner-actions">
      <button class="notif-banner-btn secondary" onclick="dismissNotifBanner()">Позже</button>
      <button class="notif-banner-btn primary" onclick="requestNotifPermission()">Включить</button>
    </div>
  </div>
...
<input type="file" id="cameraInput" class="camera-input" accept="image/*" capture="environment" onchange="handlePhoto(event)">
    ...
    <div class="main-menu-content">
      ...
      <div class="menu-section no-border">
        ...
        <div class="menu-about" onclick="closeMainMenu(); openAbout();" style="cursor: pointer;">
          ...
          <div class="menu-about-ver">v0.9.64 by Syntrise</div>
  ...
  <!-- Modals -->
  <div class="overlay" id="aboutModal"><div class="modal"><div class="about-brand"><div class="about-logo"><svg viewBox="0 0 24 24" fill="white"><path d="M12 2C12 2 5 10.5 5 15C5 18.866 8.134 22 12 22C15.866 22 19 18.866 19 15C19 10.5 12 2 12 2Z"/></svg></div><div class="about-name">DropLit</div><div class="about-ver">v0.9.64 by Syntrise</div></div><div class="modal-text" style="text-align:center">Voice-first idea capture.<br>Long press card to select & merge.</div><div class="modal-actions"><button class="modal-btn pri" onclick="closeAbout()">Got it</button></div></div></div>
...
function render(){
  ...
  for(const d of dates){
    ...
    for(const i of dayIdeas){
      ...
      const creatorName = creator === 'aski' ? 'ASKI' : creator === 'system' ? 'System' : 'Benedict Cumberbatch';
      ...
      h+='<div class="card'+(isActive?' active':'')+(isEditing?' editing':'')+(isSelected?' selected':'')+'" data-id="'+i.id+'" onclick="handleCardClick('+i.id+',event)" ontouchstart="cardTouchStart('+i.id+',event)" ontouchmove="cardTouchMove(event)" ontouchend="cardTouchEnd()" ontouchcancel="cardTouchEnd()">';
      h+='<div class="card-checkbox">'+(isSelected?'✓':'')+'</div>';
      // Header: Avatar left, two rows right
      h+='<div class="card-header">';
      h+='<div class="card-avatar '+creator+'">'+avatarSvg+'</div>';
      h+='<div class="card-header-content">';
      // Row 1: Name + Time
      h+='<div class="card-row1">';
      h+='<span class="card-creator">'+creatorName+'</span>';
