...
(function() {
  ...
  async function speak(text, options = {}) {
    ...
    // Guard: skip empty/whitespace-only text
    if (!text || !text.trim()) {
      console.log('[Kokoro] Empty text after clean, calling onEnd');
      if (onEnd) onEnd();
      return true;
    }
    ...
    // Safety timeout — if stream hangs, force cleanup
    const safetyTimer = setTimeout(() => {
      if (!completed) {
        console.warn('[Kokoro] Safety timeout — forcing onEnd');
        isSpeaking = false;
        completed = true;
        if (onEnd) onEnd();
      }
    }, Math.max(30000, text.length * 100)); // min 30s, scale with text length
  ...
  function pushText(text) {
    ...
    if (!text || !text.trim()) return; // Skip empty/whitespace after emoji strip
